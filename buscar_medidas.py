...
# 1. Cargar archivo local de medidas (para no gastar búsquedas en los que ya tenemos)
medidas = {}
if os.path.exists(MEDIDAS_FILE):
    with open(MEDIDAS_FILE, "r", encoding="utf-8") as f:
        medidas = json.load(f)

# 2. Descargar códigos del Excel
...
for codigo in codigos:
    if codigo not in medidas and nuevos < 80:  # Límite de seguridad por ejecución
        print(f"Buscando medidas para: {codigo}")
        # Hacemos una búsqueda enfocada en catálogos y dimensiones
        ...
        try:
            ...
            snippet = ""
            # Buscamos en el cuadro de respuestas de Google primero
            if "answer_box" in data and "snippet" in data["answer_box"]:
                snippet = data["answer_box"]["snippet"]
            # Si no, buscamos en el primer resultado orgánico
